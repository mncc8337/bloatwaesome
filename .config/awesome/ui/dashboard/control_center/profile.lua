...
update_fetch_component(line_wm, "", "AwesomeWM", beautiful.profile_wm)

local function update_info()
    awful.spawn.easy_async_with_shell(". /etc/os-release && printf \"$NAME\"", function(distro)
        update_fetch_component(line_distro, "", distro:sub(1, -2), beautiful.profile_distro)
    end)
    awful.spawn.easy_async("uname -r", function(version)
        update_fetch_component(line_version, "", version:sub(1, -2), beautiful.profile_version)
    end)
    awful.spawn.easy_async_with_shell("pacman -Qq | wc -l", function(package)
        update_fetch_component(line_packages, "󰏖", package:sub(1, -2), beautiful.profile_package)
    end)
end
